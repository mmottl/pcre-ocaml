...
static int pcre_callout_handler(pcre_callout_block *cb) {
  ...
  if (cod != NULL) {
    ...
    if (Is_exception_result(v_res)) {
      ...
      if (Field(v_exn, 0) == *pcre_exc_Backtrack)
        return 1;
      cod->v_exn = v_exn;
...
raise_pcre_error(value v_arg) CAMLnoreturn_end;

CAMLnoreturn_start static inline void raise_partial(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void raise_bad_partial(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void raise_bad_utf8(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void
raise_bad_utf8_offset(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void raise_match_limit(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void
raise_recursion_limit(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void
raise_workspace_size(void) CAMLnoreturn_end;

CAMLnoreturn_start static inline void
raise_bad_pattern(const char *msg, int pos) CAMLnoreturn_end;
...
CAMLprim value pcre_set_imp_match_limit_recursion_stub_bc(value v_rex,
                                                          value v_lim) {
...
#define MAKE_INTNAT_INFO(tp, name, option)                                     \
  CAMLprim intnat pcre_##name##_stub(value v_rex) {                            \
    ...
    if (ret != 0)                                                              \
      raise_internal_error("pcre_##name##_stub");                              \
    return options;                                                            \
  }                                                                            \
                                                                               \
...
CAMLprim value pcre_firsttable_stub(value v_rex) {
  ...
  if (ret != 0)
    raise_internal_error("pcre_firsttable_stub");
  CAMLreturn((ftable == NULL) ? Val_none
                              : caml_alloc_some(caml_alloc_initialized_string(
                                    32, (char *)ftable)));
}
...
CAMLprim value pcre_lastliteral_stub(value v_rex) {
  ...
  if (ret != 0)
    raise_internal_error("pcre_lastliteral_stub");
  if (lastliteral == -1)
    return Val_none;
  if (lastliteral < 0)
    raise_internal_error("pcre_lastliteral_stub");
  else
    return caml_alloc_some(Val_int(lastliteral));
}
...
CAMLnoreturn_start static inline void
handle_exec_error(char *loc, const int ret) CAMLnoreturn_end;
...
CAMLprim value pcre_exec_stub0(intnat v_opt, value v_rex, intnat v_pos,
...

  {
    ...
    if (Is_none(v_maybe_cof)) {
      ...
      /* Performs the match */
      if (is_dfa)
        ret = pcre_dfa_exec(code, extra, ocaml_subj, len, pos, opt, ovec,
                            ovec_len, (int *)&Field(v_workspace, 0),
...
static struct custom_operations tables_ops = {
    "pcre_ocaml_tables",        pcre_dealloc_tables,
    custom_compare_default,     custom_hash_default,
    custom_serialize_default,   custom_deserialize_default,
    custom_compare_ext_default, custom_fixed_length_default};
...
CAMLprim value pcre_names_stub(value v_rex) {
  ...
  if (ret != 0)
    raise_internal_error("pcre_names_stub: namecount");

  ret = pcre_fullinfo_stub(v_rex, PCRE_INFO_NAMEENTRYSIZE, &entry_size);
  if (ret != 0)
    raise_internal_error("pcre_names_stub: nameentrysize");

  ret = pcre_fullinfo_stub(v_rex, PCRE_INFO_NAMETABLE, &tbl_ptr);
  if (ret != 0)
    raise_internal_error("pcre_names_stub: nametable");
